...
add_action('wp_enqueue_scripts', function() {
    wp_enqueue_style('evfp-style', plugin_dir_url(__FILE__) . 'css/style.css', [], '1.0.0');
    wp_enqueue_script('evfp-script', plugin_dir_url(__FILE__) . 'js/script.js', [], '1.0.0', true);
    wp_localize_script('evfp-script', 'evfpData', [
        'ajax_url' => admin_url('admin-ajax.php'),
        'nonce' => wp_create_nonce('evfp_nonce'),
    ]);
});

add_filter('plugin_action_links_email-verification-for-products/email-verification-for-products.php', 'evfp_plugin_action_links');
function evfp_plugin_action_links($links) {
    $settings_link = '<a href="' . admin_url('options-general.php?page=email-verification-gate') . '">Settings</a>';
    array_unshift($links, $settings_link);
    return $links;
}

add_filter('plugin_row_meta', 'evfp_plugin_row_meta', 10, 2);
function evfp_plugin_row_meta($links, $file) {
